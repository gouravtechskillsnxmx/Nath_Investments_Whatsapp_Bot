...
async def _clear_conversation_policy_numbers_on_startup():
    """One-time safety: ensure existing conversations do not carry stale policy_number values.
    This prevents wrong-policy replies when customers have multiple policies.
    """
    try:
        db = SessionLocal()
        db.execute(update(InboxConversation).values(policy_number=None))
        db.commit()
    except Exception:
        try:
            db.rollback()
...
async def _startup_policy_decouple():
    await _clear_conversation_policy_numbers_on_startup()
app = FastAPI(title="Policy Lookup + Dashboard + Team Inbox + WhatsApp Delivery (Single File)")



# --- Policy/phone decoupling: never expose or persist policy_number on InboxConversation ---
def _sanitize_conversation_payload(d: dict) -> dict:
    try:
        if isinstance(d, dict) and 'policy_number' in d:
            d['policy_number'] = None
    except Exception:
        pass
    return d
...
def _ensure_inbox_conversation(db: Session, *, customer_phone: str, customer_name: str | None, policy_number: str | None):
  ...
  if not conv:
    conv = InboxConversation(
      ...
      policy_number=None,  # decoupled from phone; user must type policy number
      status="OPEN",
      priority="NORMAL",
  ...
  try:
    ...
    for _entry in (data.get("entry") or []):
      for _change in (_entry.get("changes") or []):
        ...
        # NOTE: policy_number is no longer stored on the conversation (decoupled from phone).
...
def admin_inbox_conversation_detail(conversation_id: str, db: Session = Depends(get_db)):
  ...
  return {
    "conversation": {
      ...
      "policy_number": None,  # decoupled from phone/conversation
      "status": conv.status,
...
async def admin_broadcast_premium_excel(file: UploadFile = File(...), dry_run: bool = False, db: Session = Depends(get_db)):
  ...
  for i, row in enumerate(ws.iter_rows(min_row=2), start=2):
    try:
      ...
      msg = _premium_reminder_message(policy_number=None,  # decoupled from phone; user must type policy number due_date=due_date, amount=amount)
      ...
      audit(db, channel="WHATSAPP", request_id=None, action="BULK_PREMIUM_REMINDER", policy_number=None,  # decoupled from phone; user must type policy number customer_phone=customer_phone, success=True, reason=f"ROW:{i}")
 ...
 async function loadConvDetail(){
  ...
  const header = `
   <div class="hint" style="margin:0 0 10px;">
    ...
    <span class="mono">${conv.customer_phone}</span> • Channel: <span class="mono">${safe(conv.channel)}</span>
   </div>
  `;
