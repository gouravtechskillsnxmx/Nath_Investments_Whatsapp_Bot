...
def openai_generate_reply(*, customer_phone: str, customer_name: str | None, user_text: str, policy_number: str | None, db: Session) -> str:
  ...
  if opt == "1":
    ...
    "Nath Investments® is an Insurance & Investment Consultancy with 25+ years of experience, "
    "led by Shashi Nath Thakur (Life Time Advisor – MDRT, COT, TOT – USA).\n\n"
    "🔹 We are experts in:\n\n"
    "• Health Insurance\n"
...
# =========================================================
# SAFE FREE-TIER BILLING PROTECTION (ADD-ONLY)
# - Conversation counting (24h window) + daily limits
# - Tenant usage guardrails (per-tenant caps)
# - Blocks outbound sends when limits are exceeded (without crashing webhook)
# =========================================================

# Enable/disable guardrails without touching code
BILLING_GUARD_ENABLED = os.getenv("BILLING_GUARD_ENABLED", "1").strip().lower() not in ("0", "false", "no", "off")
BILLING_MODE = os.getenv("BILLING_MODE", "SAFE_FREE_TIER").strip().upper()  # SAFE_FREE_TIER or PAID
TENANT_ID = os.getenv("TENANT_ID", "default").strip() or "default"

# Daily caps (tune per your demo/free offer)
DAILY_MAX_CONVERSATIONS = int(os.getenv("DAILY_MAX_CONVERSATIONS", "50"))
DAILY_MAX_OUTBOUND_MESSAGES = int(os.getenv("DAILY_MAX_OUTBOUND_MESSAGES", "250"))
DAILY_MAX_UNIQUE_CUSTOMERS = int(os.getenv("DAILY_MAX_UNIQUE_CUSTOMERS", "150"))
DAILY_MAX_OUTBOUND_PER_CUSTOMER = int(os.getenv("DAILY_MAX_OUTBOUND_PER_CUSTOMER", "25"))

CONVERSATION_WINDOW_HOURS = int(os.getenv("CONVERSATION_WINDOW_HOURS", "24"))

# Metrics keys
_METRIC_CONVERSATIONS = "conversations"
_METRIC_OUTBOUND = "outbound_messages"
_METRIC_UNIQUE = "unique_customers"

def _guard_on() -> bool:
  return bool(BILLING_GUARD_ENABLED) and (BILLING_MODE == "SAFE_FREE_TIER")

def _tenant() -> str:
  return TENANT_ID or "default"

class BillingDailyMetric(Base):
  __tablename__ = "billing_daily_metrics"
  id = Column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
  tenant_id = Column(Text, nullable=False, index=True)
  day = Column(Date, nullable=False, index=True)
  metric = Column(Text, nullable=False, index=True)
  value = Column(Numeric(18, 0), nullable=False, default=0)
  updated_at = Column(DateTime, default=now_utc, nullable=False)

class BillingCustomerWindow(Base):
  # One row per tenant+customer with a rolling 24h window (WhatsApp billing 'conversation' approximation).
  # Each time we send OUTBOUND, if now > window_end -> we open a new window and count a 'conversation'.
  __tablename__ = "billing_customer_windows"
  id = Column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
  tenant_id = Column(Text, nullable=False, index=True)
  customer_phone = Column(Text, nullable=False, index=True)
...
class BillingDailyCustomerSeen(Base):
  # Tracks unique customers per day (tenant-scoped).
  __tablename__ = "billing_daily_customers_seen"
  id = Column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
  tenant_id = Column(Text, nullable=False, index=True)
  day = Column(Date, nullable=False, index=True)
  customer_phone = Column(Text, nullable=False, index=True)
  first_seen_at = Column(DateTime, default=now_utc, nullable=False)

class BillingDailyCustomerOutbound(Base):
  # Per-customer outbound count per day (tenant-scoped).
  __tablename__ = "billing_daily_customer_outbound"
  id = Column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
  tenant_id = Column(Text, nullable=False, index=True)
  day = Column(Date, nullable=False, index=True)
  customer_phone = Column(Text, nullable=False, index=True)
  outbound_count = Column(Numeric(18, 0), nullable=False, default=0)
  updated_at = Column(DateTime, default=now_utc, nullable=False)

def _get_metric(db: Session, *, tenant_id: str, day: date, metric: str) -> int:
  row = db.execute(
    select(BillingDailyMetric)
    .where(BillingDailyMetric.tenant_id == tenant_id, BillingDailyMetric.day == day, BillingDailyMetric.metric == metric)
    .limit(1)
  ).scalars().first()
  try:
...
def _inc_metric(db: Session, *, tenant_id: str, day: date, metric: str, delta: int = 1) -> int:
  row = db.execute(
    select(BillingDailyMetric)
    .where(BillingDailyMetric.tenant_id == tenant_id, BillingDailyMetric.day == day, BillingDailyMetric.metric == metric)
    .limit(1)
  ...
  if not row:
    row = BillingDailyMetric(tenant_id=tenant_id, day=day, metric=metric, value=0, updated_at=now_utc())
    db.add(row)
    db.flush()
  try:
    row.value = (row.value or 0) + int(delta)
  except Exception:
    row.value = int(delta)
...
def _mark_unique_customer(db: Session, *, tenant_id: str, day: date, customer_phone: str) -> bool:
  existing = db.execute(
    select(BillingDailyCustomerSeen)
    .where(
      BillingDailyCustomerSeen.tenant_id == tenant_id,
      BillingDailyCustomerSeen.day == day,
      BillingDailyCustomerSeen.customer_phone == customer_phone,
    )
    .limit(1)
  ).scalars().first()
  if existing:
    return False
  db.add(BillingDailyCustomerSeen(tenant_id=tenant_id, day=day, customer_phone=customer_phone, first_seen_at=now_utc()))
  _inc_metric(db, tenant_id=tenant_id, day=day, metric=_METRIC_UNIQUE, delta=1)
  return True

def _inc_customer_outbound(db: Session, *, tenant_id: str, day: date, customer_phone: str, delta: int = 1) -> int:
  row = db.execute(
    select(BillingDailyCustomerOutbound)
    .where(
      BillingDailyCustomerOutbound.tenant_id == tenant_id,
      BillingDailyCustomerOutbound.day == day,
      BillingDailyCustomerOutbound.customer_phone == customer_phone,
    )
    .limit(1)
  ).scalars().first()
  if not row:
    row = BillingDailyCustomerOutbound(tenant_id=tenant_id, day=day, customer_phone=customer_phone, outbound_count=0, updated_at=now_utc())
    db.add(row)
    db.flush()
  try:
    row.outbound_count = (row.outbound_count or 0) + int(delta)
  except Exception:
    row.outbound_count = int(delta)
  row.updated_at = now_utc()
  db.add(row)
  try:
    return int(row.outbound_count or 0)
  except Exception:
    return 0

def _touch_conversation_window(db: Session, *, tenant_id: str, customer_phone: str, now: datetime) -> bool:
  # Returns True if a NEW conversation window was opened (counts as 'conversation').
  win = db.execute(
    select(BillingCustomerWindow)
    .where(BillingCustomerWindow.tenant_id == tenant_id, BillingCustomerWindow.customer_phone == customer_phone)
    .limit(1)
  ).scalars().first()

  window_hours = max(1, int(CONVERSATION_WINDOW_HOURS))
  window_delta = timedelta(hours=window_hours)

  if not win:
    win = BillingCustomerWindow(
      tenant_id=tenant_id,
      customer_phone=customer_phone,
      window_start=now,
    ...
    db.add(win)
    _inc_metric(db, tenant_id=tenant_id, day=now.date(), metric=_METRIC_CONVERSATIONS, delta=1)
    return True

  expired = False
  try:
    expired = (win.window_end is None) or (now > win.window_end)
  except Exception:
    expired = True

  if expired:
    win.window_start = now
    win.window_end = now + window_delta
    win.last_outbound_at = now
    win.updated_at = now_utc()
    db.add(win)
    _inc_metric(db, tenant_id=tenant_id, day=now.date(), metric=_METRIC_CONVERSATIONS, delta=1)
    return True
...
def _billing_block_reason(db: Session, *, tenant_id: str, day: date, customer_phone: str) -> str | None:
  conv = _get_metric(db, tenant_id=tenant_id, day=day, metric=_METRIC_CONVERSATIONS)
  outb = _get_metric(db, tenant_id=tenant_id, day=day, metric=_METRIC_OUTBOUND)
  uniq = _get_metric(db, tenant_id=tenant_id, day=day, metric=_METRIC_UNIQUE)

  if DAILY_MAX_CONVERSATIONS > 0 and conv >= DAILY_MAX_CONVERSATIONS:
    return f"DAILY_MAX_CONVERSATIONS reached ({conv}/{DAILY_MAX_CONVERSATIONS})"
  if DAILY_MAX_OUTBOUND_MESSAGES > 0 and outb >= DAILY_MAX_OUTBOUND_MESSAGES:
    return f"DAILY_MAX_OUTBOUND_MESSAGES reached ({outb}/{DAILY_MAX_OUTBOUND_MESSAGES})"
  if DAILY_MAX_UNIQUE_CUSTOMERS > 0 and uniq >= DAILY_MAX_UNIQUE_CUSTOMERS:
    return f"DAILY_MAX_UNIQUE_CUSTOMERS reached ({uniq}/{DAILY_MAX_UNIQUE_CUSTOMERS})"
  ...
  try:
    c_out = int(c_row.outbound_count) if c_row and c_row.outbound_count is not None else 0
  except Exception:
    c_out = 0
  if DAILY_MAX_OUTBOUND_PER_CUSTOMER > 0 and c_out >= DAILY_MAX_OUTBOUND_PER_CUSTOMER:
    return f"DAILY_MAX_OUTBOUND_PER_CUSTOMER reached ({c_out}/{DAILY_MAX_OUTBOUND_PER_CUSTOMER})"

  return None

def _record_outbound_success(db: Session, *, tenant_id: str, now: datetime, customer_phone: str):
  _mark_unique_customer(db, tenant_id=tenant_id, day=now.date(), customer_phone=customer_phone)
  _inc_metric(db, tenant_id=tenant_id, day=now.date(), metric=_METRIC_OUTBOUND, delta=1)
  _inc_customer_outbound(db, tenant_id=tenant_id, day=now.date(), customer_phone=customer_phone, delta=1)
  _touch_conversation_window(db, tenant_id=tenant_id, customer_phone=customer_phone, now=now)

def _guarded_outbound_send(kind: str, to_number: str, do_send_fn):
  if not _guard_on():
    return do_send_fn()
  ...
  with SessionLocal() as _db:
    try:
      # Ensure tables exist for this deployment
      Base.metadata.create_all(bind=engine)

      reason = _billing_block_reason(_db, tenant_id=t, day=day, customer_phone=to_number)
      if reason:
        try:
          audit(
            _db,
        ...
        try:
          ...
          pass
        return {"blocked": True, "reason": reason, "mode": BILLING_MODE, "tenant_id": t}

      res = do_send_fn()

      try:
        _record_outbound_success(_db, tenant_id=t, now=now, customer_phone=to_number)
        audit(
          _db,
          channel="WHATSAPP",
          request_id=None,
          action="BILLING_GUARD_ALLOW",
          policy_number=None,
          customer_phone=to_number,
          success=True,
          reason=f"{kind}:OK",
      ...
      # fail-open (never break business reply)
      try:
        _db.commit()
      except Exception:
        pass
...
# -----------------------
# Monkey-patch outbound sends (ADD-ONLY, no variable renames)
# -----------------------
try:
  _ORIG_send_whatsapp_text = send_whatsapp_text
  def send_whatsapp_text(to_number: str, body: str) -> dict:  # noqa: F811
    return _guarded_outbound_send("TEXT", to_number, lambda: _ORIG_send_whatsapp_text(to_number, body))

  _ORIG_send_whatsapp_image = send_whatsapp_image
  def send_whatsapp_image(to_number: str, *, image_path: str, caption: str | None = None) -> dict:  # noqa: F811
    return _guarded_outbound_send("IMAGE", to_number, lambda: _ORIG_send_whatsapp_image(to_number, image_path=image_path, caption=caption))

except Exception:
  pass

# -----------------------
# Admin endpoints for usage visibility (no dashboard UI changes required)
# -----------------------
...
@app.get("/admin/billing/usage", response_model=BillingUsageResponse)
def admin_billing_usage(db: Session = Depends(get_db)):
  t = _tenant()
  today = now_utc().date()
  metrics = {
    _METRIC_CONVERSATIONS: _get_metric(db, tenant_id=t, day=today, metric=_METRIC_CONVERSATIONS),
    _METRIC_OUTBOUND: _get_metric(db, tenant_id=t, day=today, metric=_METRIC_OUTBOUND),
    _METRIC_UNIQUE: _get_metric(db, tenant_id=t, day=today, metric=_METRIC_UNIQUE),
  }
  return BillingUsageResponse(
    ...
    day=today,
    limits={
      "DAILY_MAX_CONVERSATIONS": DAILY_MAX_CONVERSATIONS,
      "DAILY_MAX_OUTBOUND_MESSAGES": DAILY_MAX_OUTBOUND_MESSAGES,
      "DAILY_MAX_UNIQUE_CUSTOMERS": DAILY_MAX_UNIQUE_CUSTOMERS,
      "DAILY_MAX_OUTBOUND_PER_CUSTOMER": DAILY_MAX_OUTBOUND_PER_CUSTOMER,
...
def admin_billing_usage_history(days: int = 7, db: Session = Depends(get_db)):
  t = _tenant()
  ...
  rows = db.execute(
    select(BillingDailyMetric)
    .where(BillingDailyMetric.tenant_id == t, BillingDailyMetric.day >= start, BillingDailyMetric.day <= today)
    .order_by(asc(BillingDailyMetric.day), asc(BillingDailyMetric.metric))
  ).scalars().all()

  by_day = {}
  for r in rows:
    d = r.day.isoformat() if r.day else ""
    by_day.setdefault(d, {})[r.metric] = int(r.value or 0)
  ...
  for i in range(days):
    ...
    out.append({
      "day": d,
      "conversations": int(m.get(_METRIC_CONVERSATIONS, 0) or 0),
      "outbound_messages": int(m.get(_METRIC_OUTBOUND, 0) or 0),
      "unique_customers": int(m.get(_METRIC_UNIQUE, 0) or 0),
    })
  return {"ok": True, "tenant_id": t, "mode": BILLING_MODE, "items": out}
